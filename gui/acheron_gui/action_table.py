...
def build_table_row(
    client, config: dict, profile: str, inp: str, on_change: Callable[[], None], ui_state: dict
) -> Gtk.Expander:
    ...
    expander.set_child(build_binding_editor(client, config, profile, inp, on_change))
...
def build_action_table(client, config: dict, profile: str, on_change: Callable[[], None], ui_state: dict) -> Gtk.Widget:
    ...
    def render_rows():
        ...
        for inp in ALL_INPUTS:
            if inp not in base and not show_all.get_active():
                continue
            listbox.append(build_table_row(client, config, profile, inp, on_change, ui_state))
