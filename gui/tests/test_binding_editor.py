...
def test_clicking_an_unbound_key_opens_editor_defaulted_to_fire_once_keypress():
    ...
    btn = make_input_button(stub, stub.get_config(), "Default", "grid_r1c1", lambda: changed.append(1))
...
def test_saving_a_keypress_binding_calls_set_binding_and_closes_popover():
    ...
    assert stub.calls == [
        ("set_binding", "grid_r1c1", {"trigger": "fire_once", "type": "keypress", "key": "KEY_F1", "modifiers": []})
    ]
    assert changed == [1]
...
def test_clearing_an_existing_binding_calls_clear_binding():
    ...
    assert stub.calls[-1] == ("clear_binding", "grid_r1c1")
    assert changed == [1]
...
def test_bound_input_shows_bound_css_class_and_summary():
    ...
    btn = make_input_button(stub, stub.get_config(), "Default", "grid_r1c1", lambda: None)
