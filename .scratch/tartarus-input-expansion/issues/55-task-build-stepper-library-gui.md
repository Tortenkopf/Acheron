...
`build_steppers_panel` replaces the ticket-52-era `build_steppers_stub`, mirroring `build_macros_panel`'s list-plus-editor shape with two settled differences (ticket 31 round 2's Answer) plus one deliberate parity choice:

- **Delete is gated exactly like Macro's**, by direct user instruction (see below) — disabled with a "Used by N Binding(s) — can't delete" tooltip via a new `stepper_used_by_count`, mirroring `macro_used_by_count` verbatim.
...
### Delete-gate parity, corrected by direct user instruction

The first pass took this ticket's "no delete gate, unlike Macro" text at face value for the *GUI* treatment specifically (button never pre-emptively disabled, relying on the Daemon's real `DeleteStepper` refusal to surface as a post-click error) — reconciling it against the actually-landed Daemon behavior (which does refuse while referenced, per ticket 54). The user read that reconciliation and called it "a purely semantical problem": Stepper delete should behave *exactly* like Macro's regardless of the ticket text's phrasing — greyed out while referenced, not just rejected after the fact. Changed accordingly: `stepper_used_by_count` (new, mirrors `macro_used_by_count` verbatim) gates `delete_btn.set_sensitive`/tooltip in `build_stepper_row` exactly like `build_macro_row` already does. The click handler's own `DaemonError`→`show_error` wrapping was left in place (Macro's has the same belt-and-suspenders handling despite its own gate) rather than removed.
