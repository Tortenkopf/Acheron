...
- **Design/prototype/build ticket** for reusable Macro entities — expected to graduate once [Design reusable Macro entities](./issues/15-decide-reusable-macro-entities.md) resolves (decide first, then design/prototype, then build against real hardware). Chord, Stepper, mouse-button/keyboard output, and Profile Switch have all graduated: their decide tickets resolved and spawned build/prototype tickets ([Prototype the Chord recording UX](./issues/30-prototype-chord-recording-ux.md); [Prototype the Stepper library and list-editing UX](./issues/31-prototype-stepper-library-ux.md), itself blocked on ticket 15; [Prototype the key/mouse-button picker UX](./issues/32-prototype-key-mouse-button-picker-ux.md); [Build Profile Switch](./issues/34-task-build-profile-switch.md)).
