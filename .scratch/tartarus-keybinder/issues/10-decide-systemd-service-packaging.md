...
2. **GUI-ensures, as a safety net** — on its own launch, the GUI calls `org.freedesktop.systemd1.Manager.ResetFailedUnit("acheron-daemon.service")` then `StartUnit("acheron-daemon.service", "replace")` over the same session D-Bus connection it already holds for the Daemon's own interface (per [Decide D-Bus interface surface](./08-decide-dbus-interface-surface.md)) — no subprocess, no `systemctl` shell-out. `StartUnit` is idempotent (a no-op if already running) so the GUI doesn't need to check state first; `ResetFailedUnit` clears a latched `failed` substate from a genuine crash so the GUI recovers the Daemon without the user touching a terminal. (Corrected during [ticket 21](./21-systemd-packaging-and-launch-safety-net.md)'s live testing: this originally said the no-argument `Manager.ResetFailed()`, which doesn't take a unit name at all — `ResetFailedUnit(s)` is the real per-unit method.)
