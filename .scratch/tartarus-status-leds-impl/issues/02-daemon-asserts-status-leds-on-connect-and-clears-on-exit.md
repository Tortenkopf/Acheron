...
- [x] Two standalone functions in the daemon's `capture/analog` module, siblings
  ...
  - Rationale for the exact bytes and "no driver mode" is settled in
    ...
    do not re-derive.
- [x] `main.rs` creates a `tokio::sync::watch::channel::<Option<StatusLeds>>(None)`
...
- [x] The `led` task loop: `rx.changed().await` → `borrow_and_update()` the latest
...
- [x] The `watch::Sender` is threaded into the dispatch task and held on its
...
- [x] **No pre-loop assertion** in `dispatch::run`'s init block — the device may
      not be present yet, and the connect edge covers present-at-startup.
- [x] The SIGTERM/SIGINT shutdown path (`relock_and_exit`) sends the all-off frame
