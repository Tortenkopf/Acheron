...
- [ ] `daemon/src/stage.rs` imports nothing from `dispatch`/`edit`/`chord`/
...
- [ ] `StageKey(Input)` — a newtype distinguishing the deep slot's keyspace from the
...
- [ ] `StageOp` — a data-only enum, one variant per emitted op: `FirePrimary` /
      `ReleasePrimary` / `RepressPrimary` / `FireDeep` / `ReleaseDeep` /
      `SuppressPrimary` / `Nothing`.
- [ ] A pure `advance` function (or equivalent) taking the combined state
  ...
  - **Quick-Skip**: layered on Handoff's mechanics via a per-press Armed→Skipped/
    ...
    runs plain Handoff.
- [ ] The disjoint-stacked-band invariant (`(Up,Down)` is structurally impossible) is
...
- [ ] A Quick-Skip timeout pair, `next_deadline`/`tick`, mirroring
      ...
      buffer drives.
- [ ] Ordering is resolved from the *arriving event's own* depth value where a
...
- [ ] Table tests per Staging mode against the exact transition tables in spec.md —
