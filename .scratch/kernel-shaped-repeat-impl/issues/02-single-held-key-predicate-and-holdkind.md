...
- [x] `pub(crate) fn single_held_key(steps: &[MacroStep]) -> Option<(Modifiers, KeyCode)>`
...
- [x] `enum HoldKind { SustainedNoRepeat(KeyCode), AutorepeatKey(Modifiers, KeyCode) }`
...
- [x] `fn hold_repeat_kind(action: &Action, macros: &HashMap<MacroId, MacroDef>) -> Option<HoldKind>`
...
- [x] `#[allow(dead_code)]` (or equivalent) on `hold_repeat_kind` and `HoldKind`
      with a one-line comment pointing at ticket 03 — `decide` does not call it yet.
- [x] `single_held_key` unit-tested standalone per §6.1: every qualifying shape, each
      ...
      ⇒ `None`).
- [x] `hold_repeat_kind` classification tests: keyboard Keypress (plain + modified) ⇒
      ...
      `Step` ⇒ `None`.
- [x] `cargo fmt --check`, `cargo clippy --all-targets`, daemon suite all clean —
